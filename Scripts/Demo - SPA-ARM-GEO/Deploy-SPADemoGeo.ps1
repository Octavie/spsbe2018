<#
.SYNOPSIS
	Installs SPSBE 2018 Demo SPA Geo redundant
...
.EXAMPLE
	...
	C:\PS> Deploy-SPADemo -Department MAV -Environment DEV -LocationPrimary westeurope -LocationSecondary northeurope
...
$RegionSecondary = $Region

#
#
# Creating Resource Group
#
$ResourceGroupName = ("$($Department)-RG-SPAGEO-$($Environment)-$($RegionPrimary)").ToUpper()
...
Write-Host "Deploying SPSBE2018 ARM Template. Please wait..."
$templateParameters = @{
...
#
#
# Uploading homepage to North Europe
#
#
Write-Host "Uploading custom homepage for North-Europe"
$webappName = $result.Outputs.webappNameSecondary.value
$FilePath = $PSScriptRoot + "\hostingstart-ne.html"
Upload-FileToWebApp $webappName "" $ResourceGroupName $FilePath "hostingstart.html"

Write-Host "Finished"

[Diagnostics.Process]::Start( "http://spageo-$($Environment).trafficmanager.net".ToLower() )
